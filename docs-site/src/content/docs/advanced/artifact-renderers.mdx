...
Use this page after the [Core Concepts](/guides/core-concepts/) guide and the [Examples](/guides/examples/) chat showcase. Artifacts are message parts with a typed container shape and one or more renderable `views`. The current model is `UiArtifactPart`, which wraps a `UiArtifact` with:
